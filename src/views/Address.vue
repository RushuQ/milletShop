<template>
  <div>
    ...
    <nav-breader muneName="收货地址"><span>收货地址</span></nav-breader>
        ...
        <div class="checkout-addr">
          ...
          <div class="addr-list-wrap">
              ...
              <ul>
                <li v-for="(item,index) in addressListFilter" :class="{'check':checkIndex==index}" @click="checkIndex=index;selectedAddrId=item.addressId">
                  <dl>
                    ...
                    <dd class="address">{{item.streetName}} <i class="tel"> ({{item.postCode}}) </i></dd>
                  </dl>
                ...
                <li class="addr-new">
                  <div class="add-new-inner" @click="isMdShow_address=true">
                    <i class="icon-add">
            ...
            <div class="shipping-addr-more">
              <a href="javascript:;" class="addr-more-btn up-down-btn" @click="expand" v-bind:class="{'open':limit>3}" v-if="addressList.length>3">
                更多地址
                <i class="i-up-down">
    ...
    <model :mdShow="isMdShow_del" @close="closeModal">
      <p slot="message">
        您是否确认要删除此地址?
      </p>
      <div slot="btnGroup">
    ...
    <model :mdShow="isMdShow_undel" @close="closeModal">
      <p slot="message">
        地址列表至少有一条数据，已无法继续删除。
      </p>
      <div slot="btnGroup">
    ...
    <model :mdShow="isMdShow_address" @close="closeModal">
      <div slot="message">
        <div class="form-section">
          <label class="input-label">姓名</label>
          <input class="input-text" rule="request" v-model="userName" @focus="focusIpt" @blur="blurIpt" type="text" placeholder="收货人姓名">
        </div>
        <div class="form-section ">
          <label class="input-label">手机号</label>
          <input class="input-text" rule="moblie" v-model="tel" @focus="focusIpt" @blur="blurIpt" type="text" placeholder="11位手机号">
        </div>
        <div class="form-section">
          <label class="input-label">邮政编码</label>
          <input class="input-text" rule="post" v-model="postCode" @focus="focusIpt" @blur="blurIpt" type="text" placeholder="邮政编码">
        </div>
        <div class="form-section form-name">
          <label class="input-label">收货地址</label>
          <textarea class="input-text" rule="request" v-model="streetName" @focus="focusIpt" @blur="blurIpt" placeholder="详细地址，路名或街道名称，门牌号"></textarea>
        </div>
      </div>
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="addAddress">确认</a>
        <a class="btn btn--m btn--red" href="javascript:;" @click="isMdShow_address=false">取消</a>
      </div>
    </model>
...
<style>
    ...
    .form-section.form-name .input-text {
      width: 400px;
      height: 40px;
    }
    ...
    .form-section-focus .input-label,.form-section-act .input-label {
      top: -7px;
      font-size: 12px;
      background: #fff;
      color: #ff6700;
    ...
    .form-section-act .input-label {
      color: #b0b0b0;
    }
    .form-section-focus .input-text {
      border-color: #ff6700;
    }
    .input-text.form-section-warm  {
      border-color: #f50d01 !important;
    }
    .form-section-act .input-text {
      border-color: #e0e0e0;
    }
...
<script>
  ...
  import {rulesCheck} from './../util/currency';
  ...
  export default {
    ...
    methods: {
      ...
      addAddress() {
        let itemMain = document.getElementsByClassName('input-text');
        let lastEnd = rulesCheck(itemMain);
        if(!lastEnd){
          alert('格式有误，请检查后再提交！')
          return false;
        };
      ...
      focusIpt(event) {
        event.target.parentNode.classList.add('form-section-focus');
      },
      blurIpt(event) {
        let targetObj = event.target;
        targetObj.parentNode.classList.remove('form-section-focus');
        if(targetObj.value.length){
          targetObj.parentNode.classList.add('form-section-act');
        }
      }
    }
  }
